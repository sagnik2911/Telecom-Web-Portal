...
try{
	...
	PreparedStatement ps=con.prepareStatement("update clientdet set SIMNO=?,NAME=?,CITY=?,EMAIL=?,ADDRESS=?,CONNECTION_TYPE=?,CONNECTION_STATUS=?,COUNTRY=?,PIN=? where MOB_NO='"+request.getParameter("MOB")+"'");
	ps.setString(1,request.getParameter("SIM"));
	ps.setString(2,request.getParameter("Name"));
	ps.setString(3,request.getParameter("City"));
	ps.setString(4,request.getParameter("Mail"));
	ps.setString(5,request.getParameter("Address"));
	ps.setString(6,request.getParameter("Contype"));
	ps.setString(7,request.getParameter("Constatus"));
	ps.setString(8,request.getParameter("Country"));
	ps.setString(9,request.getParameter("PIN"));
	
	ps.executeUpdate();
	ps.close();
	if(!request.getParameter("Contype").equalsIgnoreCase(request.getParameter("type")))
	{
		if(request.getParameter("type").equalsIgnoreCase("Postpaid"))
		{
			Statement st = con.createStatement();
			st.executeUpdate("delete from postpaid where MOB_NO='"+request.getParameter("MOB")+"'");
			st.close();
			
			PreparedStatement ps1=con.prepareStatement("insert into prepaid values(?,?)");
			ps1.setString(1,request.getParameter("MOB"));
			ps1.setInt(2,0);
	
			ps1.executeUpdate();
			ps1.close();
		}
		else if(request.getParameter("type").equalsIgnoreCase("Prepaid"))
		{
			Statement st = con.createStatement();
			st.executeUpdate("delete from prepaid where MOB_NO='"+request.getParameter("MOB")+"'");
			st.close();
			
			PreparedStatement ps1=con.prepareStatement("insert into postpaid values(?,?)");
			ps1.setString(1,request.getParameter("MOB"));
			ps1.setInt(2,0);
